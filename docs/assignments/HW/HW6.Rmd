...
[brain](data/Brain.csv), [colon](data/Colon.csv), [esophageal](data/Esophagus.csv), [lung](data/Lung.csv) and [oral](Oral.csv) cancers for the period 1975-2016. These data are provided from the [SEER program](https://seer.cancer.gov){target=_blank}. 
...
1. Each csv file is in the same format. Open one in a text editor like Notepad (Windows) or TextEdit (Mac) to see lines you may need to skip. (I don't suggest Excel since it has a habit of changing formating and adding columns when you save back into csv). Then read each of the five files into R. For convenience I'll refer to these datasets as `brain`, `colon`, `esophagus`, `lung` and `oral`.
    - Confirm that each dataset has 42 rows and 4 columns
...
1. Now create 3 data sets, one for all races, one for whites and one for blacks. Make sure you keep the year of diagnosis in each one. 
