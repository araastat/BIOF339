...
library(rio)
library(tidyverse)
filenames <- dir('~/Dropbox/BIOF339_Fall2019/data/GSE123519_RAW/',pattern='txt', full.names = T) # Need full paths
dd <- import_list(filenames)
```
...
process_data <- function(dat){
  dat %>% select(tracking_id, gene_id, locus:FPKM) %>% 
    mutate(gene_id = stringr::str_to_upper(gene_id)) %>% 
    separate(locus, c('chromosome','location'), sep=':') %>% 
    filter(FPKM >= 1)
}

dd2 <- map(dd, process_data)
```
...
```{r}
dd3 <- bind_rows(dd2, .id = 'sampleID')
dd3 <- dd3 %>% separate(sampleID, c('sampleID','rest'), sep='_', extra = 'merge') %>% 
  select(-rest) # fixing the names of the sampleID to remove extra verbiage
```
...
```{r}
dd_design <- import('~/Dropbox/BIOF339_Fall2019/data/GSE123519design.xlsx')
dd4 <-  left_join(dd3, dd_design, by='sampleID')
```
...
unique(dd4$Class)
dd4 <- dd4 %>% 
  separate(Class, c('Class','rest'), sep = '-', extra = 'drop') %>% 
  select(-rest)
head(dd4)
```
* The above code takes care of both the points above *
