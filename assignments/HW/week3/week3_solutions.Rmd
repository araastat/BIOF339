...
```{r}
brain <- rio::import(here('data/Brain.csv'), skip=4, check.names=T)
colon <- rio::import(here('data/Colon.csv'), skip=4, check.names=T)
esophagus <- rio::import(here('data/Esophagus.csv'), skip=4, check.names=T)
lung <- rio::import(here('data/Lung.csv'), skip=4, check.names=T)
oral <- rio::import(here('data/Oral.csv'), skip=4, check.names=T)
...
d <- rio::import_list(here(files), check.names=T)
...
for(o in organs){
  ...
  assign(o, rio::import(f, check.names=TRUE))
}
...
1. Make sure you have labeled everything properly, including titles, so that the graphical information is self-evident.
