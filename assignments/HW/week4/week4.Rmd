...
```{r setup, include=F}
knitr::opts_chunk$set(echo=F, eval=FALSE, message=F, warning=F, cache=TRUE)
library(here)
```
...
1. How many unique genes of the form PIK3C* are present in the dataset (i.e., start with the string "PIK3C")
```{r}
dat2 %>% filter(str_detect(gene_id, 'PIK3C')) %>%
    summarize(length(unique(gene_id)))
```

1. What are the relative frequencies of each PIK3C* variant among the 26 mice. Assume a missing value means the variant is not present. The table should be the proportion of the 26 mice have each of the PIK3C* variants.
```{r}
dat2 %>% filter(str_detect(gene_id, 'PIK3C')) %>%
